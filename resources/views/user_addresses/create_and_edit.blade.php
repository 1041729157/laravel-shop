...
<div class="card">
  ...
  <div class="card-body">
    ...
    <user-addresses-create-and-edit inline-template>
      ...
      <form class="form-horizontal" method="post" role="form" action="{{ route('user_addresses.update', ['user_address' => $address->id]) }}">
      @method('PUT')

      @else

      <form class="form-horizontal" method="post" role="form" action="{{ route('user_addresses.store') }}">

      @endif
      <!-- 引入 csrf token 字段 -->
      {{ csrf_field() }}
        <!-- 将 JSON 字符串解析成数组赋值给Vue组件内的 initValue -->
